...
public class AppClient{
    ...
    public static AppClient InitApp(){
        ...
        try{
            ...
            SystemUtils.storeAppProperties(prop,"Config/application.properties"); //Enregistre les config
        ...
        try {
            fwq.read(fwqFile);
            System.out.println("File d'attente récupéré : "+Colors.BLUE+fwq.toString()+Colors.RESET);
     ...
     */
    public static void createFirstApp() throws IOException {
        ...
        SystemUtils.storeAppProperties(prop,"Config/application.properties"); //Sauvegarde les Configs
    }

    /**
     * Créer les dossiers nécessaires au fonctionnement de l'application
     */
    private static void installFile() throws IOException {
        //Créer le dossier de configuration
        File config = new File("Config");
        config.mkdir();

        //Créer le fichier de properties
        File propertieFile = new File("Config/application.properties");
        propertieFile.createNewFile();

        Properties props = new Properties();
        props.setProperty("app.freq","0");
        props.setProperty("app.refresh","1");
        props.setProperty("app.fwqSerFile","Config/SerFiles/fwq.ser");
        props.setProperty("app.userSerFile","Config/SerFiles/");
        props.setProperty("app.firstLaunch","true");
        props.setProperty("app.userName","");
        props.setProperty("app.host","localhost");
        props.setProperty("app.port","8080");

        props.store(new FileWriter(propertieFile),null);

        File serFiles = new File(props.getProperty("app.userSerFile"));
